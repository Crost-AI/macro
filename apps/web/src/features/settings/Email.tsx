          ...
          <Show when={inboxes().primary}>
            {(primary) => (
              <InboxRow
                ...
                onEnableCalendar={() =>
                  void startAddInbox({ includeCalendar: true })
                }
                onRemove={() =>
                  setRemoveTarget({
                    id: primary().id,
      ...
      <div class="flex items-center justify-between gap-3 min-h-15.25 py-2 px-6">
        ...
        <div class="flex items-center gap-2 shrink-0">
          ...
          {/* Its own consent flow, since Reconnect asks for the Gmail scopes
              only. Suppressed while the inbox needs a reconnect: two consent
              buttons at once is a coin flip, and the dead grant has to be
              repaired first anyway. */}
          <Show
            when={
              calendarUiEnabled() &&
