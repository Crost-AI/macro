...
import type { SessionStatus } from '../context/create-session-status-controller';
import {
...
/** 'claude-code' → 'Claude Code'; the session has no name of its own. */
function harnessTitle(harness: string | undefined): string {
...
export function AgentSplitHeader(props: {
  ...
  const title = () => harnessTitle(props.session?.harness);
