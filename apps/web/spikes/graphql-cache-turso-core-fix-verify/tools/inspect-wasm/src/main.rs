...
struct Inspection {
    ...
    memory_grow_operator_count: u64,
    clock_time_imports: Vec<String>,
    random_crypto_imports: Vec<String>,
    filesystem_imports: Vec<String>,
...
fn inspect_bytes(
    ...
    let mut inspection = Inspection {
        ...
        memory_grow_operator_count: 0,
        clock_time_imports: Vec::new(),
        random_crypto_imports: Vec::new(),
        filesystem_imports: Vec::new(),
        ...
        match payload.map_err(|error| format!("parse WASM: {error}"))? {
            ...
            Payload::CodeSectionEntry(body) => {
                ...
                while !operators.eof() {
                    ...
                    if matches!(operator, wasmparser::Operator::MemoryGrow { .. }) {
                        inspection.memory_grow_operator_count += 1;
                    }
...
fn classify_suspicious_import(import: &Import, inspection: &mut Inspection) {
    ...
    if lower.contains("clock")
        ...
        || lower.contains("__wbg_now")
    {
        inspection.clock_time_imports.push(qualified.clone());
    }
    if lower.contains("random") || lower.contains("crypto") || lower.contains("getrandom") {
        inspection.random_crypto_imports.push(qualified.clone());
    }
    if lower.contains("filesystem")
        || lower.contains("wasi:filesystem")
        || lower.contains("__wbg_fs_")
    {
        inspection.filesystem_imports.push(qualified.clone());
    }
    if lower.contains("wasi") {
        inspection.wasi_imports.push(qualified.clone());
    }
    if lower.contains("thread") || lower.contains("atomic") || lower.contains("shared_memory") {
...
fn contract_violations(inspection: &Inspection) -> Vec<String> {
    ...
    if !inspection.wasi_imports.is_empty() {
        violations.push("found WASI-related imports".to_string());
    }
    if !inspection.thread_related_imports.is_empty() {
        violations.push("found thread/shared-memory-related imports".to_string());
    }
