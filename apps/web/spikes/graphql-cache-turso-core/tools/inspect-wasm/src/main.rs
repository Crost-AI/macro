...
fn inspect_bytes(
    ...
    if let Some(expected_imports) = expected_imports {
        inspection.unexpected_imports = inspection
            .imports
            .iter()
            .map(Import::allowlist_entry)
            .filter(|entry| !expected_imports.contains(entry))
            .collect();
        inspection.imports_allowed = Some(inspection.unexpected_imports.is_empty());
    }
...
fn contract_violations(inspection: &Inspection) -> Vec<String> {
    ...
    if inspection.imports_allowed == Some(false) {
        violations.push(format!(
            "found {} imports outside the explicit allowlist",
