...
const BEGIN_PROBE_PATH: &str = "begin-immediate.db";
const BEGIN_PROBE_WAL_PATH: &str = "begin-immediate.db-wal";
...
const BEGIN_PATHS: [PathSpec; 2] = [
    PathSpec {
        path: BEGIN_PROBE_PATH,
        direct: true,
        allow_no_lock: false,
    },
    PathSpec {
        path: BEGIN_PROBE_WAL_PATH,
        direct: false,
        allow_no_lock: true,
    },
];
...
/// Asynchronously create an owner-scoped `BEGIN IMMEDIATE` probe session.
#[wasm_bindgen]
pub async fn begin_immediate_probe_session(owner: u32) -> Result<String, JsValue> {
...
/// Reproduce Turso's built-in `MemoryIO` clock panic on wasm32.
#[cfg(feature = "failing-runtime-probes")]
#[wasm_bindgen]
pub fn run_builtin_memory_io_probe() -> Result<(), JsValue> {
...
#[cfg(feature = "failing-runtime-probes")]
#[wasm_bindgen]
pub fn run_begin_immediate_probe(owner: u32, session: u32) -> Result<(), JsValue> {
    with_database(
        OwnerId(owner),
        SessionId(session),
        BEGIN_PROBE_PATH,
