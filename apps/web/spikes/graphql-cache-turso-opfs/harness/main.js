...
async function acquireOwnerUntil({ deadlineMs = 1_000, retryDelayMs = 100 } = {}) {
...
async function runProbe() {
  ...
  try {
    ...
    assert(first.init.capabilities.dedicatedWorker, "engine did not run in a DedicatedWorker");
    assert(first.init.capabilities.storageGetDirectory, "OPFS getDirectory is unavailable");
    assert(first.init.capabilities.webLocks, "Web Locks is unavailable in DedicatedWorker");
    assert(!first.init.capabilities.crossOriginIsolated, "test unexpectedly used cross-origin isolation");
    assert(first.init.capabilities.nestedWorkerMonitorInstalled, "nested-worker monitor missing");
    assert(first.init.capabilities.nestedWorkerConstructionCount === 0, "nested worker constructed");
    ...
    report.memoryIoProbe = await expectedTrap(memoryOwner.worker, "memoryProbe");
    assert(report.memoryIoProbe.trapped, "built-in MemoryIO unexpectedly worked on wasm32");
    assert(
    ...
    report.beginImmediateProbe = await expectedTrap(immediateOwner.worker, "beginImmediateProbe");
    assert(report.beginImmediateProbe.trapped, "BEGIN IMMEDIATE unexpectedly worked on wasm32");
    const immediateStack = report.beginImmediateProbe.error.stack ?? "";
    assert(immediateStack.includes("std::time::Instant::now"), "immediate trap lost Instant cause");
    assert(
    ...
    liveWorkers = liveWorkers.filter((worker) => worker !== immediateOwner.worker);
