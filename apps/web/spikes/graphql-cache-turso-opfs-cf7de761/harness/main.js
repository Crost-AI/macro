...
class ProbeWorker {
  constructor(routeClassification = "production") {
    ...
    this.worker.addEventListener("message", (event) => {
      ...
      if (message.id !== undefined) {
        ...
        if (message.ok) {
          ...
          pending.reject(
            Object.assign(new Error(message.error.message), message.error, { runtimeEvidence }),
          );
        }
        return;
      }
      this.recordRuntimeEvidence(`event:${message.event ?? "unknown"}`, message.runtimeEvidence);
    ...
    this.worker.addEventListener("error", (event) => {
      ...
      const error = Object.assign(new Error(event.message || "worker error"), {
        wasmEnvironmentTrap: isWasmEnvironmentTrap(event.error ?? event.message),
  ...
  call(command, payload = {}, timeoutMs = 60_000) {
    ...
    return new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        ...
        reject(controlledHarnessError(`${command} timed out after ${timeoutMs}ms`));
      }, timeoutMs);
  ...
  waitForEvent(event, predicate = () => true, timeoutMs = 30_000) {
    ...
    return new Promise((resolve, reject) => {
      ...
      waiter.timeout = setTimeout(() => {
        this.eventWaiters = this.eventWaiters.filter((candidate) => candidate !== waiter);
        reject(controlledHarnessError(`event ${event} timed out after ${timeoutMs}ms`));
      }, timeoutMs);
  ...
  terminate() {
    ...
    this.rejectAll(controlledHarnessError("worker terminated while RPC pending"));
  }
}
...
function collectRecordedErrors(value, path = "report", records = []) {
  ...
  if (Array.isArray(value)) {
    value.forEach((entry, index) => collectRecordedErrors(entry, `${path}[${index}]`, records));
  } else {
...
function runtimeSafetySnapshot() {
  return {
    ...
    maxExpectedNegativeWasmTrapCount: Math.max(
      0,
      ...workerRuntimeObservations.map((evidence) => evidence.expectedNegativeWasmTrapCount ?? 0),
    ),
    maxUnhandledWorkerRuntimeFailureCount: Math.max(
      0,
      ...workerRuntimeObservations.map((evidence) => evidence.unhandledRuntimeFailureCount ?? 0),
    ),
    workerRuntimeObservations: workerRuntimeObservations.map((evidence) => ({ ...evidence })),
  };
}
...
function assertHeadRuntimeSafety(report) {
  ...
  assert(expectedNegative.length === 1, "explicit-temp negative trap evidence was not unique");
  assert(expectedNegative[0].wasmEnvironmentTrap, "explicit-temp negative route did not trap");
  assert(
  ...
  assert(workerRuntimeObservations.length > 0, "worker runtime evidence was not recorded");
  assert(
  ...
  assert(
    workerRuntimeObservations
      ...
      .every(
        (evidence) => evidence.workerRouteClassification === "explicit-temp-negative",
      ) &&
      workerRuntimeObservations
        .filter((evidence) => evidence.workerRouteClassification === "explicit-temp-negative")
        .every((evidence) =>
          [
            "rpc:initOwner",
  ...
  assert(pageUnhandledRuntimeFailureCount === 0, "page observed an unhandled runtime failure");
  assert(pageWasmEnvironmentTrapCount === 0, "page observed a WASM environment trap");
...
async function recoverDatabaseUntil({
  ...
  while (performance.now() < deadline) {
    ...
    try {
      ...
      lastAttempt = { recovered: false, timedOut: /timed out/i.test(error.message), error };
    ...
    if (lastAttempt.recovered) {
      ...
      assert(elapsedMs <= deadlineMs, "full OPFS recovery completed after its deadline");
      assert(candidateTerminations === attempts, "a recovery candidate was not terminated");
      return {
        result: lastAttempt,
        recovery: {
...
async function exerciseTransactionMode(worker, mode, expectParentFailure) {
  ...
  try {
    ...
    assert(result.value.rollback_preserved, `${mode} rollback did not preserve the commit`);
    ...
    assert(record.wasmEnvironmentTrap, `parent ${mode} did not fail with a WASM trap`);
    assert(
      /std::time::Instant::now|not implemented on this platform/.test(record.stack ?? ""),
      `parent ${mode} trap lost the Instant/platform cause`,
    );
    assert(
      /ensure_temp_database|create_temp_database|open_file_with_flags/.test(record.stack ?? ""),
...
async function runProbe() {
  ...
  const expectParentFailure =
    new URL(location.href).searchParams.get("transactionExpectation") === "parent-failure";
  const runPhase = new URL(location.href).searchParams.get("runPhase") ?? "cold";
  const report = {
    startedAt: new Date().toISOString(),
    runPhase,
    transactionExpectation: expectParentFailure ? "parent-failure" : "head-success",
  ...
  try {
    assert(workersBeforeFirstUse === 0, "DedicatedWorker was constructed before first use");
    ...
    assert(capabilityOwner.init.capabilities.dedicatedWorker, "engine did not run in a DedicatedWorker");
    assert(capabilityOwner.init.capabilities.storageGetDirectory, "OPFS getDirectory is unavailable");
    assert(capabilityOwner.init.capabilities.webLocks, "Web Locks is unavailable in DedicatedWorker");
    assert(
      !capabilityOwner.init.capabilities.crossOriginIsolated,
      "test unexpectedly used cross-origin isolation",
    );
    assert(capabilityOwner.init.capabilities.nestedWorkerMonitorInstalled, "nested-worker monitor missing");
    assert(
    ...
    for (const mode of ["immediate", "exclusive"]) {
      const transactionOwner =
        mode === "immediate" ? capabilityOwner : await acquireOwnerUntil({ deadlineMs: 5_000 });
      if (transactionOwner !== capabilityOwner) liveWorkers.push(transactionOwner.worker);
      ...
      liveWorkers = liveWorkers.filter((worker) => worker !== transactionOwner.worker);
    }

    if (expectParentFailure) {
      report.differentialExpectedFailure =
        !report.transactionModes.immediate.succeeded && !report.transactionModes.exclusive.succeeded;
      assert(report.differentialExpectedFailure, "parent differential did not reproduce both failures");
    ...
    assert(direct.empty_write_bytes === 0, "empty pwrite did not complete as zero bytes");
    assert(direct.partial_write_retried, "partial pwrite was not retried");
    assert(direct.short_read_bytes === 2 && direct.eof_bytes === 0, "short/EOF read mismatch");
    assert(direct.zero_write_error === "ShortWrite", "zero write did not become ShortWrite");
    assert(direct.error_preserved && direct.quota_preserved, "specific write error was lost");
    ...
    for (const field of [
      ...
      assert(cacheSql[field] === true, `full cache SQL contract failed: ${field}`);
    }
    ...
    report.fullCacheSqlContractPass = cacheSql.foreign_key_check_violation_shape === true;
    report.fullCacheSqlKnownFailures = report.fullCacheSqlContractPass
      ? []
      : [
          {
            check: "PRAGMA foreign_key_check deliberate violation exact four-column shape",
    ...
    assert(cacheSql.foreign_key_check_rows === 0, "full cache SQL foreign_key_check failed");
    assert(
    ...
    report.sameWorkerCachePersistence = await first.worker.call("verifyFullCachePersistence");
    report.sqlWrite = await first.worker.call("sqlWrite", { value: "persisted-v1" });
    report.sameWorkerReopen = await first.worker.call("sqlRead");
    report.serializedQueue = await Promise.all([
      first.worker.call("sqlRead"),
      first.worker.call("sqlRead"),
    ]);
    assert(report.sqlWrite.value.journal_mode === "wal", "Turso did not use WAL mode");
    assert(report.sameWorkerReopen.value.value === "persisted-v1", "same-worker reopen lost SQL data");
    assert(
      report.serializedQueue.every((entry) => entry.value.value === "persisted-v1"),
    ...
    report.lifecycleFailure = await lifecycleOwner.worker.call("lifecycleFailureProbe");
    assert(report.lifecycleFailure.closeFailed, "injected close unexpectedly succeeded");
    assert(report.lifecycleFailure.deleteRejected, "delete followed an uncertain close");
    assert(report.lifecycleFailure.reopenRejected, "poisoned registry reopened");
    assert(report.lifecycleFailure.lifecycle.startsWith("poisoned:"), "registry was not poisoned");
    lifecycleOwner.worker.terminate();
    liveWorkers = liveWorkers.filter((worker) => worker !== lifecycleOwner.worker);

    const removeFailureOwner = await acquireOwnerUntil({ deadlineMs: 5_000 });
    liveWorkers.push(removeFailureOwner.worker);
    report.removeEntryFailure = await removeFailureOwner.worker.call("removeEntryFailureProbe");
    assert(report.removeEntryFailure.resetFailed, "actual removeEntry failure was not observed");
    assert(
      report.removeEntryFailure.actualRemoveEntryFailure,
      "removeEntry failure did not retain its browser error cause",
    );
    assert(report.removeEntryFailure.secondResetRejected, "poisoned remove reset retried");
    assert(report.removeEntryFailure.reopenRejected, "remove failure poison reopened");
    assert(report.removeEntryFailure.lifecycle.startsWith("poisoned:"), "remove failure did not poison");
    assert(report.removeEntryFailure.artifactCleaned, "remove failure artifact was not cleaned");
    removeFailureOwner.worker.terminate();
    liveWorkers = liveWorkers.filter((worker) => worker !== removeFailureOwner.worker);

    const recreationFailureOwner = await acquireOwnerUntil({ deadlineMs: 5_000 });
    liveWorkers.push(recreationFailureOwner.worker);
    report.recreationFailure = await recreationFailureOwner.worker.call("recreationFailureProbe");
    assert(report.recreationFailure.resetFailed, "actual recreation failure was not observed");
    assert(
      report.recreationFailure.actualRecreationFailure,
      "recreation failure did not retain its browser error cause",
    );
    assert(report.recreationFailure.secondResetRejected, "poisoned recreation reset retried");
    assert(report.recreationFailure.reopenRejected, "recreation failure poison reopened");
    assert(
      report.recreationFailure.lifecycle.startsWith("poisoned:"),
      "recreation failure did not poison",
    );
    assert(report.recreationFailure.artifactCleaned, "recreation failure artifact was not cleaned");
    recreationFailureOwner.worker.terminate();
    liveWorkers = liveWorkers.filter((worker) => worker !== recreationFailureOwner.worker);

    const second = await acquireOwnerUntil({ deadlineMs: 5_000 });
    liveWorkers.push(second.worker);
    report.crossWorkerCachePersistence = await second.worker.call("verifyFullCachePersistence");
    assert(
    ...
    assert(report.crossWorkerReopen.value.value === "persisted-v1", "cross-worker reopen lost SQL data");

    const excluded = new ProbeWorker();
    liveWorkers.push(excluded);
    const excludedInit = await excluded.call("initOwner", { lockName: LOCK_NAME });
    report.webLockExclusion = {
      ownerAcquired: true,
      contenderAcquired: excludedInit.acquired,
    };
    assert(!excludedInit.acquired, "two workers acquired the database Web Lock");
    excluded.terminate();
    liveWorkers = liveWorkers.filter((worker) => worker !== excluded);

    const killRpc = second.worker.call("killWrite", {}, 120_000);
    const firstCommit = await second.worker.waitForEvent("kill-first-commit", () => true, 30_000);
    assert(firstCommit.commitCount === 1, "kill event was not the first successful commit");
    assert(
    ...
    assert(
      firstCommit.postSizes["graphql-cache.db"] >= firstCommit.preSizes["graphql-cache.db"],
    ...
    assert(pendingRpcRejected, "kill RPC did not reject after worker termination");
    report.workerKill = {
    ...
    assert(report.abruptReset.committed.committed_rows >= 1, "first commit was not durable");
    assert(
      report.abruptReset.committed.committed_rows < report.abruptReset.committed.finite_bound,
    ...
    assert(abruptBefore["graphql-cache.db"] > 0, "recovery pre-open saw no database file");
    assert(abruptBefore["graphql-cache.db-wal"] > 0, "recovery pre-open saw no WAL file");
    assert(abruptReset.deleted["graphql-cache.db"], "abrupt reset did not delete main");
    assert(abruptReset.deleted["graphql-cache.db-wal"], "abrupt reset did not delete WAL");
    assert(
    ...
    assert(report.freshRecovery.value.count_before === 0, "fresh recovery retained old SQL rows");
    ...
    report.explicitTempNegativeReset = await negativeOwner.worker.call("resetTransactionProbe");
    ...
    assert(
      report.explicitTempNegative.runtimeEvidence.productionReachableWasmTrapCount === 0 &&
        report.explicitTempNegative.runtimeEvidence.expectedNegativeWasmTrapCount === 1 &&
        report.explicitTempNegative.runtimeEvidence.unhandledRuntimeFailureCount === 0 &&
        report.explicitTempNegative.runtimeEvidence.workerRouteClassification ===
          "explicit-temp-negative",
      "explicit temp negative worker counters were not isolated",
    );
    negativeOwner.worker.terminate();
    liveWorkers = liveWorkers.filter((worker) => worker !== negativeOwner.worker);

    report.workerConstructionCount = workerConstructionCount;
    report.noNestedWorker = {
      runtimeMonitorInstalled: true,
      constructionCount: report.firstShutdown.evidence.nestedWorkerConstructionCount,
