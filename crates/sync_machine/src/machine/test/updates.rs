...
use crate::model::{
    Caps, ClientFrame, CloseReason, ConnId, Effect, Input, RawSnapshot, RawUpdate, ServerFrame,
};
...
fn update_order_is_apply_persist_ack_broadcast() {
    ...
    let token = actions
        ...
        .unwrap();
    let actions = machine.handle(Input::OpsPersisted {
        token,
        through_seq: 1,
    });
    let ack_at = actions
...
fn viewer_updates_are_dropped_silently() {
    ...
    let actions = machine.handle(Input::Frame {
        conn: C2,
        frame: ClientFrame::Update {
            updates: vec![RawUpdate::from(&b"sneaky"[..])],
            id: "op-x".into(),
        },
    });
...
fn persist_failure_schedules_retry_and_retry_resends_the_tail() {
    ...
    let actions = machine.handle(Input::PersistFailed { token });
    let retry = actions
        .iter()
        .find_map(|action| match action {
    ...
    let actions = machine.handle(Input::TimerFired { token: retry });
    let token = actions
        .iter()
        .find_map(|action| match action {
            Effect::PersistOps { token, .. } => Some(*token),
