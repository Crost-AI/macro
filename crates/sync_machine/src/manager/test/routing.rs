...
use crate::model::{Caps, ClientFrame, ConnId, DocId, Effect, Lifecycle, RawSnapshot, ServerFrame};
...
fn per_doc_detaches_reach_their_documents() {
    ...
    let mut actions = manager.handle(ManagerInput::Detach {
        conn: ConnId(1),
        doc: DocId("doc-a".to_string()),
    });
    actions.extend(manager.handle(ManagerInput::Detach {
        conn: ConnId(2),
        doc: DocId("doc-b".to_string()),
    }));
    let last_leaves: Vec<&DocId> = actions
        .iter()
        .filter(|action| {
            matches!(
                action.effect,
    ...
    assert_eq!(
        last_leaves,
        vec![&DocId("doc-a".to_string()), &DocId("doc-b".to_string())]
    );
}
