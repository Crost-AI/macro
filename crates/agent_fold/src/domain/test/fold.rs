...
fn synthesizes_a_write_diff_from_raw_input() {
    ...
    let messages = fold(log);
    let MessagePart::ToolUse(write) = &messages[1].parts[0] else {
        panic!("first part is the write: {:?}", messages[1].parts);
    };
    let ToolDetail::Edit { diffs } = &write.detail else {
        panic!("write folds to an edit: {:?}", write.detail);
    };
    assert_eq!(diffs.len(), 1);
...
fn reported_diffs_beat_the_synthesized_write_diff() {
    ...
    let messages = fold(log);
    let MessagePart::ToolUse(edit) = &messages[1].parts[0] else {
        panic!("first part is the edit: {:?}", messages[1].parts);
    };
    let ToolDetail::Edit { diffs } = &edit.detail else {
        panic!("edit folds to diffs: {:?}", edit.detail);
    };
    assert_eq!(diffs.len(), 1);
