...
4. **One patch series on `main`.** `main` = `<upstream-tag>` + Crost commits. The current upstream base tag is recorded in `.upstream-base`.
...
- `git range-diff` output shows how Crost deltas move relative to the tag change.
